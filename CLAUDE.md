...
No auth/accounts. No photo upload. No admin panel (read the DB directly). No email drip sequences. No native apps. No animation libraries. And in this phase: no new features without funnel data or customer requests demanding them, the two known candidates are in-workspace paragraph editing and a Lob delivery-status webhook.

**No multi-state, with an explicit gate (set 2026-08-01):** expand to a second state only once Massachusetts produces **10+ paying customers in a single month through a channel that is repeatable at positive contribution margin.** As of 2026-08-01 there is exactly **one** real customer, and she came from paid search that loses about $112 per acquisition, so nothing is validated yet. The reasoning: the bottleneck is customer acquisition economics, not addressable market (MA alone has ~2M renter households and a huge Sept 1 turnover). Adding a state duplicates the acquisition problem while doubling the legal surface that must stay correct, and the legal work, not the code, is the expensive part. `lib/statute/ma.ts` really is isolated so a second state is a new file, but each state needs its own verified statute, deadlines, remedies, consumer-protection analog, and ongoing case-law tracking (MA alone required finding and incorporating Peebles a year after it was decided). A wrong rule means mailing legally incorrect demand letters in a customer's name.
