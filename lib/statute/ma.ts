...
export function analyzeTenancy(
  ...
  rules.push({
    ...
    explanation: r7Triggered
      ? "§15B(2)(c) requires the landlord to furnish a signed, separate written statement of the premises' condition upon receiving the deposit or within 10 days after the tenancy began, whichever is later. This does not by itself forfeit the deposit, but failing to provide one is a separate unfair or deceptive practice under 940 CMR 3.17(4)(e). It also undercuts any move-out damage claim: in Peebles v. JRK Property Holdings, Inc., SJC-13702 (Aug. 1, 2025), the Supreme Judicial Court treated the unit's condition at the start of the lease as one of the circumstances deciding whether damage is reasonable wear and tear, and noted (n.7) that this statement is the document the Legislature provides for that question."
  ...
  // Named for what the predicate actually knows: contestableCount comes from
  // R5's keyword matcher, which flags "nail hole", "scuff", "dust", "faded", etc,
  // not just cleaning or painting charges, and runs before LEGITIMATE_KEYWORDS
  // (so e.g. "smoke damage cleaning" matches "clean"). Never state a match as fact.
  const contestableChargePresent = contestableCount > 0;
  rules.push({
    ...
    severity: r8Triggered ? (contestableChargePresent ? "medium" : "low") : "info",
  });
